...
def test():
    ...
    display = Display(spi, dc=Pin("LCD_D_C"), cs=Pin("LCD_CS"), rst=Pin("LCD_RESET"),width=320,height=240,rotation=90)
    x_center = display.width // 2
    y_center = display.height // 2
    # ---------------------------------------
    # GDW Added 12 November, 2023
    bkgnd = 0x039F  # Brandeis Blue
    display.display_on()
    #display.clear(bkgnd)
    backlight.on()
    # ---------------------------------------
